...
export function UserFilters({
  ...
  const hasFilters = selectedRegions.length > 0 || selectedAgeGroups.length > 0 || registrationStatus !== "all"
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
              ...
              <Button variant="outline" className="gap-2 bg-transparent">
                ...
                {hasFilters && (
                  <Badge variant="secondary" className="ml-1 px-1.5 min-w-[20px] h-5">
                    {selectedRegions.length + selectedAgeGroups.length + (registrationStatus !== "all" ? 1 : 0)}
                  </Badge>
                )}
              </Button>
            </PopoverTrigger>
                  ...
                  <div className="mt-2 space-y-2">
                    {ageGroups.map((ageGroup) => (
                      <div key={ageGroup} className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor={`age-${ageGroup}`} className="text-sm cursor-pointer flex-1">
                          {ageGroup}
                        </label>
                      </div>
                    ))}
                    ...
                    {[
                      ...
                      <div key={status.value} className="flex items-center gap-2">
                        <Checkbox
                          ...
                          onCheckedChange={() => onRegistrationStatusChange(status.value)}
                        />
          ...
          <Button variant="outline" onClick={onExport} className="gap-2 bg-transparent">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {selectedAgeGroups.map((ageGroup) => (
            <Badge key={ageGroup} variant="secondary" className="gap-1">
              ...
              <X
                className="h-3 w-3 cursor-pointer"
                onClick={() => onAgeGroupsChange(selectedAgeGroups.filter((a) => a !== ageGroup))}
              />
            </Badge>
          ))}
          {registrationStatus !== "all" && (
            <Badge variant="secondary" className="gap-1">
              ...
              <X className="h-3 w-3 cursor-pointer" onClick={() => onRegistrationStatusChange("all")} />
            </Badge>
          )}
        </div>
      )}
